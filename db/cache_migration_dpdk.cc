...
inline int CacheMigrationDpdk::TxMainPipeline(void* arg) {
  ...
  CacheMigrationDpdk* self = static_cast<CacheMigrationDpdk*>(ctx->instance);

  if (!self) {
    std::cerr << "Error: No instance pointer in TxMainPipeline!" << std::endl;
    return -1;
  }
  ...
  // 批量大小调优：64为10G/25G网络的最佳点
  constexpr uint16_t BATCH_SIZE = 64;
  rte_mbuf* batch_packets[BATCH_SIZE];

  // 获取key模板数量
  size_t num_templates = self->key_templates_.size();
  if (num_templates == 0) {
    std::cerr << "Error: No key templates generated!" << std::endl;
    return -1;
  }
  ...
  size_t request_id = start;
  while (request_id < end && running.load(std::memory_order_acquire)) {
    // 1. 批量分配mbuf
    uint16_t batch_count = std::min<uint16_t>(BATCH_SIZE, end - request_id);
    uint16_t alloced = rte_pktmbuf_alloc_bulk(self->tx_mbufpool_, batch_packets, batch_count);
    if (unlikely(alloced != batch_count)) {
      // 分配失败，释放已分配的并重试
      for (uint16_t i = 0; i < alloced; i++) {
        rte_pktmbuf_free(batch_packets[i]);
      }
    ...
    for (uint16_t i = 0; i < batch_count; i++) {
      size_t req_idx = request_id + i;

      // 设置请求开始时间
      requests[req_idx].start_time = get_now_micros();

      // 选择key模板（循环使用）
      const KeyTemplate& key_template = self->key_templates_[req_idx % num_templates];

      // 构造包
      rte_mbuf* mbuf = batch_packets[i];
      char* pkt_data = rte_pktmbuf_append(mbuf, TOTAL_LEN);
      if (unlikely(!pkt_data)) {
        std::cerr << "Error: Failed to append packet data" << std::endl;
        rte_pktmbuf_free(mbuf);
        batch_packets[i] = nullptr;
        continue;
      }
      ...
      // ETH头
      rte_ether_hdr* eth_hdr = reinterpret_cast<rte_ether_hdr*>(pkt_data);
      rte_ether_addr_copy(&self->s_eth_addr_, &eth_hdr->src_addr);
      rte_ether_addr_copy(&self->d_eth_addr_, &eth_hdr->dst_addr);
      eth_hdr->ether_type = rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4);
      ...
      ip_hdr->next_proto_id = IPPROTO_UDP;
      ip_hdr->time_to_live = 64;
      ip_hdr->src_addr = self->dev_infos_[0].src_ip;
      ip_hdr->dst_addr = key_template.dst_ip;
      ip_hdr->hdr_checksum = 0;
      ip_hdr->hdr_checksum = rte_ipv4_cksum(ip_hdr);
      ...
      // KV请求头
      KVRequest* kv_req = reinterpret_cast<KVRequest*>(udp_hdr + 1);
      uint8_t dev_id = self->dev_infos_.empty() ? 0 : self->dev_infos_[0].dev_id;
      kv_req->dev_info = static_cast<uint8_t>(ENCODE_DEV_INFO(dev_id, DEV_CLIENT));
      kv_req->request_id = rte_cpu_to_be_32(req_idx);
      // 根据req_idx决定操作类型
      uint8_t op = (req_idx % 2 == 0) ? READ_REQUEST : WRITE_REQUEST;
      kv_req->combined = static_cast<uint8_t>(ENCODE_COMBINED(CLIENT_REQUEST, op));
      rte_memcpy(kv_req->key.data(), key_template.data, KEY_LENGTH);
      // 填充value
      if (op == READ_REQUEST) {
        rte_memcpy(kv_req->value1.data(), "READ", 4);
      } else {
        rte_memcpy(kv_req->value1.data(), "WRITE", 5);
      }
    ...
    // 3. 批量发送
    uint16_t nb_tx = rte_eth_tx_burst(0, queue_id, batch_packets, batch_count);
    tx_success_count += nb_tx;
    tx_drop_count += (batch_count - nb_tx);

    // 4. 释放mbuf（发送失败的）
    for (uint16_t i = nb_tx; i < batch_count; i++) {
      if (batch_packets[i]) {
        rte_pktmbuf_free(batch_packets[i]);
        size_t failed_req_id = request_id + i;
        requests[failed_req_id].completed.store(true);
        false_count.fetch_add(1, std::memory_order_relaxed);
      }
    }

    request_id += batch_count;
  }

  uint64_t elapsed_us = get_now_micros() - send_start_us;
  double qps = (tx_success_count * 1000000.0) / elapsed_us;

  std::cout << "[Pipeline TX] Core " << rte_lcore_id()
            << " sent " << tx_success_count << " drop " << tx_drop_count
