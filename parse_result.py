...
def main():
    if len(sys.argv) != 2 or sys.argv[1] == '-h':
        print "Usage: %s OutputFileName" % sys.argv[0] # type: ignore
        print "Statistics (.result file) for each workload " + "will be written to the workload directory." # type: ignore
    ...
    for line in lines:
        ...
        if not results.has_key(workload): # type: ignore
            results[workload] = {}
